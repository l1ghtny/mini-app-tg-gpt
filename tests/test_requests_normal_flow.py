...
async def test_text_request_reserve_and_consume(monkeypatch):
    ...
    async with AsyncSession(engine, expire_on_commit=False) as s:
        user = m.AppUser(telegram_id=721000001)
        s.add(user)
        await s.commit()
        await s.refresh(user)
        convo = m.Conversation(user_id=user.id, title="T", model="gpt-5.4-nano")
        s.add(convo)
        await s.commit()
    ...
    async with AsyncSession(engine, expire_on_commit=False) as s:
        ...
        assert rl and rl.state == app.db.models.State.consumed
