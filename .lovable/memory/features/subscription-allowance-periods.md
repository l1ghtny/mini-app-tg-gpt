...
`subscription_anchor` marks aligned accounts. Legacy alignment keeps the same account ID and balances with a zero-unit audit event. Multiple overlapping legacy rows require reconciliation. Both prod and beta share the same scope. The database `allowance_control` gate starts in calendar compatibility mode; only after all writers are updated may the release operator pause, drain and enable subscription periods atomically. It verifies balances and rejects rollback to calendar mode after alignment. Supplier operational budgets stay calendar-based.
